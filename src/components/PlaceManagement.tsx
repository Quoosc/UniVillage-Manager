...
const Badge = ({
  ...
  const styles: any = {
    orange:
      "bg-orange-50 text-orange-700 border-orange-100 ring-orange-600/20",
    blue: "bg-blue-50 text-blue-700 border-blue-100 ring-blue-600/20",
    green:
      "bg-green-50 text-green-700 border-green-100 ring-green-600/20",
    purple:
      "bg-purple-50 text-purple-700 border-purple-100 ring-purple-600/20",
    gray: "bg-gray-50 text-gray-700 border-gray-100 ring-gray-600/20",
    red: "bg-red-50 text-red-700 border-red-100 ring-red-600/20",
  };
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ring-1 ring-inset ${styles[color] || styles.gray}`}
    >
      {children}
    </span>
  );
...
const initialPlaces: Place[] = [
  // Pending Places
  {
    id: "P001",
    name: "Hồ Đá",
    address: "Khu đô thị ĐHQG, Đông Hòa, Dĩ An",
    thumbnail:
  ...
  {
    id: "P002",
    name: "Tiệm Photo Tuấn",
    address: "45 Đường Lý Thường Kiệt, Khu A",
    thumbnail:
  ...
  {
    id: "P003",
    name: "Quán Cà Phê Guitar Mộc",
    address: "78 Đường Vành Đai, KTX Khu B",
    thumbnail:
  ...
  // Approved Places (Active)
  {
    id: "P004",
    name: "Cơm Tấm Kiều Giang",
    address: "Cổng chính KTX Khu A",
    thumbnail:
...
export function PlaceManagement() {
  ...
  const handleApprove = (place: Place) => {
    if (confirm(`Xác nhận duyệt địa điểm: ${place.name}?`)) {
      setPlaces((prev) =>
        prev.map((p) =>
          p.id === place.id ? { ...p, status: "Open" } : p,
        ),
      );
      // Tùy chọn: Chuyển tab sang list để xem kết quả
      // setActiveTab('list');
    }
  };
  ...
  const handleConfirmReject = () => {
    if (selectedPlace) {
      setPlaces((prev) =>
        prev.filter((p) => p.id !== selectedPlace.id),
      ); // Xóa khỏi danh sách
      // Hoặc nếu muốn giữ lại lịch sử thì set status = 'Rejected'
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-50/50">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Kiểm duyệt đề xuất và quản lý dữ liệu bản đồ
            </p>
          </div>
        ...
        {/* ================= CONTENT: PENDING LIST (TABLE VIEW) ================= */}
        {activeTab === "pending" && (
            ...
            {filteredPlaces.length > 0 ? (
                    ...
                    {filteredPlaces.map((place) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <img
                              src={place.proposer.avatar}
                              alt={place.proposer.name}
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end items-center gap-2">
                            <button
                              ...
                              className="group/btn flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-700 hover:bg-green-100 hover:text-green-800 rounded-lg transition-all border border-green-200 shadow-sm"
                            >
                              <Check
                                size={16}
                                strokeWidth={2.5}
                            ...
                            <button
                              onClick={() =>
                                openRejectModal(place)
                              }
                              className="group/btn flex items-center gap-1.5 px-3 py-1.5 bg-white text-red-600 hover:bg-red-50 hover:text-red-700 rounded-lg transition-all border border-gray-200 hover:border-red-200 shadow-sm"
                            >
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-gray-500 mt-1 max-w-sm">
                  Tuyệt vời! Bạn đã xử lý hết các địa điểm chờ
                  duyệt.
                </p>
              </div>
            )}
        ...
        {/* ================= CONTENT: ACTIVE LIST (GRID CARD VIEW) ================= */}
        {activeTab === "list" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            {filteredPlaces.length > 0 ? (
              filteredPlaces.map((place) => (
                <div
                  key={place.id}
                  className="group bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col"
                >
                  {/* Image Header */}
        ...
        <div className="space-y-4">
          <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-700 mt-1">
                Hành động này sẽ xóa địa điểm khỏi danh sách chờ
                duyệt và gửi thông báo đến người đề xuất.
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="VD: Thông tin địa điểm không chính xác, Hình ảnh mờ, Địa điểm trùng lặp..."
            ></textarea>
          </div>
...
// Helper component for fallback image (Inline to ensure single file functionality)
const ImageWithFallback = ({
  ...
  return (
    <img
      src={
        error
          ? "https://images.unsplash.com/photo-1560520653-9e0e4c89eb11?w=200&h=200&fit=crop"
          : src
      }
      alt={alt}
